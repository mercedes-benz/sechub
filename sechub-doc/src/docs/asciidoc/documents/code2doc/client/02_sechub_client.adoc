...
Does a synchronous scan which means

 - create a `{sechub}` job
 - upload necessary stuff to `{sechub}` (e.g. zipped sourcefiles for source scans)
 - approve job as ready to start
 - wait until the job is done,
 - download the report file
 - exits non-zero on `red` (critical or high findings) so your build will break (depends on your build tool)
...
See here for details: <<section-client-configuration-file,Configuration file>> changeme!!!
----
...
TIP: Defaults to latest job. +
     You can select the job by providing the wanted job UUID by providing `-jobUUID ${jobUUID}`
...
TIP: This might be valuable for an overview +
     or when you want to see your project's scan job queue +
     or find out the job UUID of a certain job.
...
Removes formerly defined false positives entries. +
