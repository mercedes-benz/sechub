...
Does a synchronous scan which means
 ...
 - create a `{sechub}` job
 - upload necessary data to `{sechub}` (e.g. zipped sourcefiles for source scans)
 - approve job as ready to start
 - wait until the job is done,
 - download the report file
 - exits with a non-zero status on `red` (critical or high findings) so your build will break (depends on your build tool)
