...
===== markFalsePositives
Read your report carefully and check which of the findings are really so called "false positives" (FP).
After this create an json file containing jobUUID and ID of finding you decided to be a FP. If 
...
- `y` Yes - Mark as false positive. You are prompted for an optional comment line which can help later to understand why this has been marked.
...
- `n` No - Do nothing and go to next item.
- `s` Skip following findings - End the asking here and remove the marked items from the false positives list on the sechub server.
