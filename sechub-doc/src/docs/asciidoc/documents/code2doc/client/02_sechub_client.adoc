...
It's very easy to use and using the client you do not have handle REST-API calls by yourself.
...
- commandline options (override below)
...
- `SECHUB_KEEP_TEMPFILES` +
  .zip files for code scan will not be removed, so you can check, what was uploaded to the `{sechub}` server
