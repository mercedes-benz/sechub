...
NOTE: Providing an empty list here does *NOT delete* the declared false positives! +
Since `markFalsePositives` only adds false positives, an empty `jobData` list has no effect at all. (don't do this)
...
- `-reportformat <format>` +
  Output format for reports. Currently supported: [html,json]. If not a supported format, "json" will always be the fallback.
- `-server <url>` +
