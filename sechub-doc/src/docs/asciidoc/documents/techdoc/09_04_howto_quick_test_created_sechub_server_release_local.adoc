...
[[section-howto-quck-test-server-release-local]]
...
[NOTE]
====
Here an example for a scratch-build: Checked out gi tag `v0.19.1-server` and by executing `./gradlew bootJar`) you will
find the server release 0.19.1 at `sechub-server/build/libs/sechub-server-0.19.1.jar` 
====

==== Start release at your machine  
