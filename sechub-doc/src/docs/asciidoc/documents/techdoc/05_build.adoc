...
=== Build stages
Builds stages were introduced because of a problematic life cycle situation when it comes to Java API generation. 
...
In a nutshell: At gradle initialization phase the API related projects are only added when wanted.
...
For eclipse build ship you must enter the following command and import the additionally created
projects manually into eclipse:
