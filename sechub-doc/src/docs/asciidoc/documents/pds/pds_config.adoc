...
|PDS_JOB_RESULT_FILE                | The absolute path to the result file (must contain security output)
|PDS_JOB_SOURCECODE_ZIP_FILE        | The absolute path to the uploaded `sourcecode.zip file` 
|PDS_JOB_SOURCECODE_UNZIPPED_FOLDER | When auto unzipping is enabled (default) the uploaded sourcecode was unzipped to this folder 
|===
