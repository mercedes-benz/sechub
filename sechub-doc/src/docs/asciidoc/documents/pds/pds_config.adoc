...
|PDS_JOB_USER_MESSAGES_FOLDER       a| The absolute path to the user messages folder.
...
would overwrite the old ones...
====

Here a solution for bash to create unique names:
...
Mandatory parts are always present, optional ones can be empty - depending on the executor configuration. 
