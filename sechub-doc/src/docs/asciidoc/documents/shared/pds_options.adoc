...
===== Logging
====== Default output
{pds} uses per default logging to stdout, so you just need to use something like
`fluentd` PODs to gather log output.

====== Logstash support
When you want to use `logstash` (e.g. when using `Kibana`) you just have to set the environment variable 
