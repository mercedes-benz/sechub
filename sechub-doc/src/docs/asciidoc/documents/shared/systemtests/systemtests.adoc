...
So it is normally not necessary to explicitly define much to have a running system test!
...
tests there by a remote setup. In this setup it is not possible to change any existing 
configuration. This means that executor configurations, users, projects, profiles etc. 
must be configured on server side by administrators.
...
an administrative account, but only a normal user account which has access to the project.
...
inside the stage, but it is possible change the defaults inside the configuration. Here is an example:
