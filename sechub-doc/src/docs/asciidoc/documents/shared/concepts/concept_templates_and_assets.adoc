...
GET    /api/admin/template/$template-id
DELETE /api/admin/template/$template-id
```

PUT will contain a body with the template data:
...
----
<1> The type of the template. Must be set when a template is created. Will
...
<5> Describes if the variable is optional.  +
...
<6> Variable content validation definition ((optional)
<7> Minimum length (optional)
<8> Maximum length (optional)
<9> Regular expression (optional). If defined, the content must match the given regular expression
    to be valid. 
...
GET     /api/admin/project/$projectId/templates
```
...
[IMPORTANT]
====
For templates and assets we must use shared storage between {sechub} and {pds} .
{pds} solutions may not to provide own storage for {pds} - different storages 
...
as filenames when storing asset data inside database and storage. The product identifiers
...
- upload asset ZIP files by REST API and create checksum file +
  `PUT /api/administration/asset/$assetId/file/$filename`
- download asset files by REST API  +
  `GET /api/administration/asset/$assetId/file/$filename`
- list (ZIP) file names +
  `GET /api/administration/asset/$assetId/list`
- delete single asset file by REST API  +
  `DELETE /api/administration/asset/$assetId/file/$filename`
...
{pds} parameter : `pds.template.metadata` with JSON:
...
WebScanTemplateData templateData = util.fetchWebScanTemplateData(sechubConfig);
if (templateData!=null){

    // folder=/$workspaceFolder/assets
    File folder = util.getAssetFolder();
    
    // fetch file: in example it is a web scan login template
    File scriptFile = folder.getChild(ID_WEBSCAN_LOGIN+"/script.js");
    
    // process
    String script = readTextFile(scriptFile);
    script = replaceVariableInScript(templateData.getVariable("username"), script);
    script = replaceVariableInScript(templateData.getVariable("password"), script);
