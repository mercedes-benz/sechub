...
- be a spring boot application with using network DB (like done in SecHub itself, so we have t his
...
- has got a common simple way for configuration (at the beginning: ENV entry containing JSON)
