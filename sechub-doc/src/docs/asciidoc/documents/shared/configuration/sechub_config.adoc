...
TIP: In most cases it is enough to specify a `seed` for the TOTP configuration in SecHub.
For the optional values `validityInSecods`, `tokenLength` and `hashAlgorithm` there are defaults which will be used if this values are not specified.
This defaults are well-known and any authentication provider or application should provide the optional data if it uses something other than the defaults.
