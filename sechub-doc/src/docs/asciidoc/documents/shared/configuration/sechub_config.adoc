...
Instead of configure the filesystem and upload local data, you can also define a remote data section to scan data from remote locations.
...
The `type` is not mandatory but recommended to be set to `git` or `docker` to make it clear what kind of remote data is used.
...
[source, json, title="Codescan scan configuration for remote git sources without credentials"]
----
include::sechub_config_example18_remote_data_without_credentials_sources_codescan.json[]
----
<1> `location`: The URL to your remote data. (for example to a git repository or a docker registry)
<2> `type`: Describe your remote data. Can currently supported are `git` or `docker`.

[[sechub-config-example-remote-data-section-sources-codescan-credentials]]
====== Example source remote data scan with credentials
[source, json, title="Codescan scan configuration for remote git sources with credentials"]
----
...
<4> `name`: The username you wish to use to access your remote data
<5> `password`: The API token that grant your username access to your remote data
...
----
<1> `location`: The URL to your remote docker image. (for example to a docker registry)
<2> `type`: Describe your remote data. Can currently supported are `git` or `docker`.
