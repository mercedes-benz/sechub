...
===== Define Remote Data Section to scan Data from remote locations
...
Sechub will download the data from the remote location and provide it to the PDS for scanning.
...
<2> `type`: Describe your remote data. Can currently be `git` or `docker`.
...
----
<1> `location`: The URL to your remote data.
<2> `type`: Describe your remote data. Can currently be `git` or `docker`.
