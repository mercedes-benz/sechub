...
To make the scan work the target URL will always be implicitly included with `"https://www.gamechanger.example.org<*>"`.
If you want to scan only a specific part of your web application you need to specify the target URL with more details like `"https://www.gamechanger.example.org/path/to/scan/target"`.
<3> *Optional*: Define includes, if you have a special path that is linked nowhere,
	...
	You can use wildcards by using the symbol `<*>` like in the example above.
