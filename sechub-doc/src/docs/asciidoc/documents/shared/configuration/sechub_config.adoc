...
<6> Optional Parameter: If `"sensitive" : true`, the value of this header will be shown inside the SecHub report. 
This means, if `"sensitive" : false`, the value of this header will be inside the report.
The **default** value is `"sensitive" : true` if nothing is specified.
