...
Here some links to examples which are using <<sechub-config-data-section,data>> inside their configurations:
...
[NOTE]
====
The next figure shows a full blown example with all possible attributes defined.
It's is just presents all possibilities. And of course the arrays can contain multiple settings .
====

[source, json, title="General JSON syntax"]
----
include::sechub_config_data_section_simple_general_example.json[]
----
...
<3> `name`:  Is a unique name for the data configuration element. The name must
    be unique inside complete sechub configuration file! 
