...
All source code files inside the defined folders will be automatically zipped and uploaded by `{sechub}` client
to server side for source code analysis.

Pathes must be defined relative to caller location - for example
...
<5> The {sechub} client only adds "well known" sources into the upload zip file.
    This removes unnecessary stuff and so reduces upload size and scan duration.
