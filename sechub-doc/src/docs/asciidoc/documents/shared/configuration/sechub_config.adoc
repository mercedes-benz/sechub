...
Specifying the header name `x-sechub-dast` with a custom value inside the JSON scan configuration, will overwrite the the default value which is the SecHub jobUUID.
