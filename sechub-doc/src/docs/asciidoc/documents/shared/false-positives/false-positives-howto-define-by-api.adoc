...
- To be a false positive a finding the `cweId` and the `urlPattern`. A list of (HTTP) `methods` can be specified to limit the false positive. If no `methods` are specified, this false positive entry will not be restricted to any method. +
...
- A `urlPattern` which consists of only wildcards (`pass:[*]`) is not allowed.
