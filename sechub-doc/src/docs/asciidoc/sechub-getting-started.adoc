...
* How to start a SecHub environment and Gitleaks+PDS using Helm charts and container images
...
The SecHub server is responsible for taking jobs and passing them to the PDS server(s) for processing. Once the job finishes on the PDS side, they are fetched and processed by SecHub.
...
./20_deploy_pds-gitleaks.sh
./40_apply-profiles.sh
--

This starts the SecHub server and a PDS with Gitleaks secret scanner.

=== Setup a project in SecHub

The following call will create a SecHub project named "demo" and assigns the profile "secretscanner-gitleaks" to it:

[source, shell]
--
./50_create_SecHub_project.sh demo secretscanner-gitleaks
--
...
After starting all servers, some environment variables have to be set. Specifically `SECHUB_SERVER`, `SECHUB_USERID`, `SECHUB_APITOKEN`, `SECHUB_TRUSTALL`. Make sure that the SecHub client executable "sechub" is in the PATH.
...
We have prepared mini-example in the "demo" folder.

Please have a look at the `sechub.json` there, which tells which files to be scanned, and which to be skipped.
...
This will run a Gitleaks scan via SecHub and create a file which contains a report of the findings from the scan (JSON format).
