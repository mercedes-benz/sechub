...
After running it, you should get similar output if everything is working corretly:
[source]
--
user: gosec
project: test-gosec
...
And to install it, open VsCode and in the `Command Pallete` (Usually can be oppened with `Ctrl+Shift+P`) type `install vsix`, and in the pop-up menu, choose the plugin.