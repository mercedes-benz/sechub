...
public class UseCaseAsciiDocGenerator {
    ...
    private void generateEventTraceOverviewIfPreGenerated(Context context, UseCaseEntry entry) {
        ...
        String subFolderPath = UseCaseEventOverviewPlantUmlGenerator.createPlantumlFileSubPathByUsecase(entry.getIdentifierEnumName());
        File overviewFile = new File(context.diagramsGenFolder, subFolderPath);
        if (! overviewFile.exists()) {
            LOG.warn("Event-Trace MISSING:{} - No event-trace overview found at:{}",entry.getIdentifierEnumName(), overviewFile.getAbsolutePath());
            return;
        }
        LOG.info("Event-Trace FOUND  :{} - Event-trace overview found  at:{}", entry.getIdentifierEnumName(), overviewFile.getAbsolutePath());
        
        context.addLine("*Event overview*");
        context.addLine("");
        context.addLine("plantuml::diagrams/gen/"+subFolderPath+"[format=svg, alt=\"Overview of events happening at usecase "+entry.getIdentifierEnumName()+"]\n");
        context.addLine("");
    }
