...
public class DocGeneratorUtil {
	...
	static Class<?> fetchClass(AnnotatedElement element) {
		...
		/* worst case should never happen... at least do error output and return the annotation class itself*/
		/* NOSONAR */System.err.println("fetch class not possible for element:"+element+".\nFallback to element class itself!");
		return element.getClass();
	}

	/**
	 * Reduces string to second upper cased char - e.g. "NetsparkerInstallSetupImpl" would be replaced to "Netsparker"
	 * @param clazz
	 * @return
	 */
