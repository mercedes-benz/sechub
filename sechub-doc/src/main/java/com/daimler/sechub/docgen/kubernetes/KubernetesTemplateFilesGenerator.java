...
import com.daimler.sechub.docgen.spring.SpringScheduleExtractor;
import com.daimler.sechub.docgen.spring.SpringScheduleExtractor.SpringSchedule;
import com.daimler.sechub.docgen.spring.SpringValueExtractor;
...
class KubernetesTemplateFilesGenerator implements Generator {
	...
	public void generate(KubernetesFiles result, List<DocAnnotationData> list) throws IOException {
		...
		/* config (normally unnecessary because automatical generated, but we want .json as file ending, so here necessary */
		...
		File generatedDeploymentFilePart = new File(ensureKubernetesGenFolder(),
				"sechub-server-environment-deployment-parts.gen.yaml");
	...
	private void generateDeploymentCode(KubernetesFiles result, DocAnnotationData data) {
		...
		if (data.isSecret) {
			...
			if (springValue != null && !springValue.isEmpty()) {
				...
				newLine(code, "          value: \""
						+ springScheduledExtractor.extract(data.springScheduled).getScheduleDefaultValue() + "\"");
			}
		}

	}

	private void generateSecretShellScriptsAndMissingSecretFiles(KubernetesFiles result)
			throws IOException {
		createSecretShellScriptAndFiles(result, ensureSecretFolder());
	}

	private void createSecretShellScriptAndFiles(KubernetesFiles result, File targetFolder)
			throws IOException {
		...
		StringBuilder sb = new StringBuilder();
		// add robot parts. deploy-secrets-secret-sechub-robot.sh is manual created and exists
		// only sechub-robot-secret.yaml must copied to templates folder manual and is not inside GIT
