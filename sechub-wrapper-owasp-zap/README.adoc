...
===== MissingMandatoryBindingException example usage
There is a custom Exception called `MissingMandatoryBindingException` that can be used inside the script,
e.g. if it is necessary to send a dedicated `SechubMessage` about an incorrect SecHub configuration.
The message of the exception will be passed on as a `SechubMessage`.
