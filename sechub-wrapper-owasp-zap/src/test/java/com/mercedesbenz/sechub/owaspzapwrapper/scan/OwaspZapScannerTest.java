...
import com.mercedesbenz.sechub.owaspzapwrapper.scan.OwaspZapScanner.UserInformation;
import com.mercedesbenz.sechub.owaspzapwrapper.util.SystemUtil;
import com.mercedesbenz.sechub.test.TestFileReader;

class OwaspZapScannerTest {

    private OwaspZapScanner scannerToTest;

    private ClientApiFacade clientApiFacade;
    private OwaspZapScanContext scanContext;
    private OwaspZapEventHandler owaspZapEventHandler;
    private SystemUtil systemUtil;

    private OwaspZapProductMessageHelper helper;
    ...
    void beforeEach() {
        ...
        scanContext = mock(OwaspZapScanContext.class);
        systemUtil = mock(SystemUtil.class);
        helper = mock(OwaspZapProductMessageHelper.class);

        owaspZapEventHandler = mock(OwaspZapEventHandler.class);

        // assign mocks
        scannerToTest = new OwaspZapScanner(clientApiFacade, scanContext);
        scannerToTest.systemUtil = systemUtil;
        scannerToTest.owaspZapEventHandler = owaspZapEventHandler;

        // set global behavior
        when(scanContext.getContextName()).thenReturn(contextName);
        when(scanContext.getOwaspZapProductMessageHelper()).thenReturn(helper);
        ...
        doNothing().when(systemUtil).waitForMilliseconds(OwaspZapScanner.CHECK_SCAN_STATUS_TIME_IN_MILLISECONDS);
    ...
    void deactivate_rules_ruleset_or_rules_to_deactivate_null_results_in_nothing_is_configured() throws ClientApiException {
        ...
        scannerToTest.deactivateRules(new OwaspZapFullRuleset(), null);
        scannerToTest.deactivateRules(null, new DeactivatedRuleReferences());
        scannerToTest.deactivateRules(new OwaspZapFullRuleset(), deactivatedReferences);
    ...
    void deactivate_rules_results_in_rules_are_deactivated() throws ClientApiException {
        ...
        String json = TestFileReader.loadTextFile("src/test/resources/zap-available-rules/owaspzap-full-ruleset.json");
        OwaspZapFullRuleset ruleSet = new OwaspZapFullRuleset().fromJSON(json);
    ...
    void setup_addtional_proxy_information_results_in_proxy_enabled() throws ClientApiException {
        ...
        when(clientApiFacade.configureHttpProxy(host, "" + port, null, null, null)).thenReturn(null);
        ...
        /* test */
        verify(clientApiFacade, times(1)).configureHttpProxy(host, "" + port, null, null, null);
    ...
    void set_includes_and_excludes_api_facade_is_called_once_for_each_include_and_once_for_exclude(String sechubConfigFile)
        ...
        IncludeExcludeToOwaspZapURLHelper helper = new IncludeExcludeToOwaspZapURLHelper();

        URL targetUrl = sechubWebScanConfig.getUrl().toURL();
        List<String> includesList = sechubWebScanConfig.getIncludes().get();
        Set<URL> includes = new HashSet<>(helper.createListOfUrls(OwaspZapURLType.INCLUDE, targetUrl, includesList, new ArrayList<>()));
        when(scanContext.getOwaspZapURLsIncludeSet()).thenReturn(includes);

        List<String> excludesList = sechubWebScanConfig.getExcludes().get();
        Set<URL> excludes = new HashSet<>(helper.createListOfUrls(OwaspZapURLType.EXCLUDE, targetUrl, excludesList, new ArrayList<>()));
        when(scanContext.getOwaspZapURLsExcludeSet()).thenReturn(excludes);
        ...
        verify(clientApiFacade, times(includes.size())).accessUrlViaOwaspZap(any(), any());
    ...
    void configure_login_inside_owasp_zap_using_no_auth_and_unsupported_auth_return_null(String sechubConfigFile) throws ClientApiException {
        /* prepare */
        String contextId = "context-id";
        String json = TestFileReader.loadTextFile(sechubConfigFile);
        SecHubWebScanConfiguration sechubWebScanConfig = SecHubScanConfiguration.createFromJSON(json).getWebScan().get();
        ...
        /* execute */
        UserInformation userInformation = scannerToTest.configureLoginInsideOwaspZapContext(contextId);
    ...
    @Test
    void configure_login_inside_owasp_zap_using_basic_auth_results_in_expected_calls() throws ClientApiException, MalformedURLException {
        /* prepare */
        String contextId = "context-id";
        ...
        when(clientApiFacade.configureAuthenticationMethod(eq(contextId), eq(AuthenticationType.HTTP_BASIC_AUTHENTICATION.getOwaspZapAuthenticationMethod()),
                any())).thenReturn(response);
        when(clientApiFacade.sessionManagementMethod(eq(contextId), eq(SessionManagementType.HTTP_AUTH_SESSION_MANAGEMENT.getOwaspZapSessionManagementMethod()),
                any())).thenReturn(response);
        ...
        assertEquals(userId, userInformation.getOwaspZapuserId());
        ...
        verify(clientApiFacade, times(1)).configureAuthenticationMethod(eq(contextId),
                eq(AuthenticationType.HTTP_BASIC_AUTHENTICATION.getOwaspZapAuthenticationMethod()), any());
        verify(clientApiFacade, times(1)).sessionManagementMethod(eq(contextId),
                eq(SessionManagementType.HTTP_AUTH_SESSION_MANAGEMENT.getOwaspZapSessionManagementMethod()), any());
	...
	void generate_report_calls_api_facade_once() throws ClientApiException {
		...
		/* execute */
		scannerToTest.generateOwaspZapReport();
    ...
    void wait_for_ajaxSpider_scan_is_cancelled_results_in_exception_with_dedicated_exit_code() throws ClientApiException {
        /* prepare */
        scannerToTest.owaspZapEventHandler = owaspZapEventHandler;
        when(owaspZapEventHandler.isScanCancelled()).thenReturn(true);
        doCallRealMethod().when(owaspZapEventHandler).cancelScan(contextName);
        ...
        verify(owaspZapEventHandler, times(2)).isScanCancelled();
    ...
    void wait_for_ajaxSpider_scan_ended_results_in_expected_calls() throws ClientApiException {
        /* prepare */
        scannerToTest.owaspZapEventHandler = owaspZapEventHandler;
        when(owaspZapEventHandler.isScanCancelled()).thenReturn(false);
    ...
    void wait_for_spider_scan_ended_results_in_expected_calls() throws ClientApiException {
        ...
        when(scanContext.getMaxScanDurationInMillis()).thenReturn(1000L);
        when(scanContext.isActiveScanEnabled()).thenReturn(true);
        OwaspZapProductMessageHelper messageHelper = mock(OwaspZapProductMessageHelper.class);
        when(scanContext.getOwaspZapProductMessageHelper()).thenReturn(messageHelper);
        ...
        verify(scanContext, times(1)).getMaxScanDurationInMillis();
        verify(scanContext, times(1)).isActiveScanEnabled();
        verify(scanContext, times(1)).getOwaspZapProductMessageHelper();
