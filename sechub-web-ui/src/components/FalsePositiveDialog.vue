        ...
        v-if="isWebScan">
          <v-table class="false-positive-webscan-pattern-table">
            <thead class="sechub-primary-color-report">
              <tr>
                <th>{{ $t('REPORT_DETAILS_WEBSCAN_METHOD') }}</th>
            ...
            <tbody>
              <tr v-for="(falsePositive, i) in webScanFalsePositives" 
              :key="i" 
              class="background-color">
              <td>
                {{ (falsePositive.methods || []).join(', ') }}
                <v-btn 
                  icon="mdi-pencil"
                  variant="text"
                  size="small"
              ...
              <td>
                ...
                <td>{{ falsePositive.cweId != -1? falsePositive.cweId : '' }}</td>
                <td>
                  <v-btn 
                  icon="mdi-close"
                  size="small"
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      async function markAsFalsePositive(){
        var falsePositive: FalsePositives = {}
        if(isWebScan.value){
        ...
        var isSuccess = await useMarkAsFalsePositive(projectId.value, falsePositive)
        if(isSuccess){
            emit('successAlert')
            emit('close', true)
        }else{
