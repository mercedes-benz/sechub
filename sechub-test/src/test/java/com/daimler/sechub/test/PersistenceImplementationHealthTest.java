	...
	public void ensure_JPAEntities_haveOptimisticLockingImplemented() {
		...
		for (Class<?> entityClass : entityClasses) {
			if (entityClass.isAnonymousClass()) {
				/* anonymous classes are ignored - cannot be real hibernate parts... so only test parts*/
				continue;
			}
	...
	public void ensure_JPAEntities_haveEqualsAndHashCodeImplemented() {
		...
		for (Class<?> entityClass : entityClasses) {
			...
			for (Method m : entityClass.getDeclaredMethods()) {
				...
				if (m.getName().equals("equals") && m.getReturnType().isPrimitive() && m.getParameterCount()==1 && m.getParameterTypes()[0].equals(Object.class)) {
