...
public class AuthUserUpdateRolesService {
	...
	/**
	 * Why this retry mechanism? The update role event can happen multiple times - so an optimistic lock may happen. But last event shall override.
	 * @param userId
	 * @param roles
	 * @param retryCount
	 */
	private void internalUpdateRoles(String userId, Set<String> roles, int retryCount) {
		...
		LOG.debug("Current auth roles of user '{}'. Roles: superadmin={}, user={}, owner={}", userId, user.isRoleSuperAdmin(),
				user.isRoleUser(), user.isRoleOwner());
		...
		LOG.info("Updated auth roles of user '{}'. Roles: superadmin={}, user={}, owner={}", userId, user.isRoleSuperAdmin(),
				user.isRoleUser(), user.isRoleOwner());
		try{
			authUserRepository.save(user);
		}catch(OptimisticLockingFailureException e) {
			/* when this happens we do not really care! reason: last event is the event that matters!*/
