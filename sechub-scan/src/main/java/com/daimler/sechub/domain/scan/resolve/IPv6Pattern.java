...
public class IPv6Pattern implements InetAddressPattern{
	...
	private void validate(String pattern) {
		...
		this.valid=false;
		if (parts.length!=8) {
			/* we do NOT support short form but only long variant, which has
			 * always 8 parts*/
			LOG.debug("Pattern '{}' not valid for IPv6 (we do not provide compact form for patterns, only long variant). Amount of dividers must be 8 and not {}.", pattern,parts.length);
			return;
		}
