...
/**
 * This entity represents only the access for users to a project.
 * As long as there are is no tupel (user + project) the access is forbidden or
 * project does not exist. Even an administrator must be added as user. So access
 * is always auditable etc. and we got also no jobs without a correct owner!
 *
