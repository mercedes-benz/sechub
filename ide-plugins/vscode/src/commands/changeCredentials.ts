...
export async function changeCredentials(sechubContext: SecHubContext): Promise<void> {
	const currentUsername =
		(await sechubContext.extensionContext.secrets.get(SECHUB_API_CLIENT_CONFIG_KEYS.username)) || '';
	const currentApiToken =
		(await sechubContext.extensionContext.secrets.get(SECHUB_API_CLIENT_CONFIG_KEYS.apiToken)) || '';
