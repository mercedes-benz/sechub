...
export async function openFindingCallStackOld(sechubContext: SecHubContext, reportItem: ReportItem): Promise<void> {
    ...
    if (reportItem instanceof ReportFindingItem) {
        ...
        sechubContext.callHierarchyTreeDataProvider.getChildren().then((hierarchyItems) => {
            ...
            vscode.commands.executeCommand(SECHUB_COMMANDS.openFinding, hierarchyItem.finding);
        });
    }
}
