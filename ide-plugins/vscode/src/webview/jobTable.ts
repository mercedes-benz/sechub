...
export class JobListTable {
	...
	private failedToFetchJobsMessage =
		'<p>Failed to retrieve job list. You are either not allowed to view them or facing server connection issues.</p>';
	...
	async renderJobTable(context: vscode.ExtensionContext): Promise<string> {
		...
		if (data) {
			...
			if (data.content && data.content.length > 0) {
				...
				const table = `${title}<table id="sechubJobTable" class="sechubTable">${haeder}${tableContent}</table>`;
