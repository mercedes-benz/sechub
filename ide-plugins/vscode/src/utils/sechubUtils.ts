...
export function getFalsePositivesByIDForJobReport(
	falsePositiveConfig: FalsePositiveProjectConfiguration,
	jobUUID: string,
): number[] {
...
export async function preSelectedProjectValid(context: vscode.ExtensionContext): Promise<void> {
	...
	if (!projects || !projects.some(p => p.projectId === project.projectId)) {
		vscode.window.showErrorMessage(
			`Selected project ${project.projectId} is not valid. Please select a valid project.`,
		);
