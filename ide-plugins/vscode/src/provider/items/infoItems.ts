...
export class WebScanInfoItem extends InfoItem {
	...
	private createWebScanDetails(): InfoItem[] {
		...
		const attackItem = new MetaDataInfoItem('Attack', '', undefined, vscode.TreeItemCollapsibleState.Collapsed);

		if (this.web.request) {
			requestItem.children.push(
				new MetaDataInfoItem(
					'Method:',
					this.web.request.protocol + ' ' + this.web.request.version + ' ' + this.web.request.method,
					undefined,
			...
			if (this.web.request.headers) {
				for (const [key, value] of Object.entries(this.web.request.headers)) {
					headers.children.push(
						new MetaDataInfoItem(key, value, undefined, vscode.TreeItemCollapsibleState.None),
					);
				}
			}
		...
		if (this.web.response) {
			responseItem.children.push(
				new MetaDataInfoItem(
					'Status Code:',
					this.web.response.protocol + ' ' + this.web.response.version + ' ' + this.web.response.statusCode,
					undefined,
					vscode.TreeItemCollapsibleState.None,
				),
			);
		...
		if (this.web.attack) {
			attackItem.children.push(
				new MetaDataInfoItem(
					'Vector:',
					this.web.attack.vector || 'N/A',
					undefined,
					vscode.TreeItemCollapsibleState.None,
				),
			);
			attackItem.children.push(
				new MetaDataInfoItem(
					'Evidence:',
					this.web.attack.evidence?.snippet || 'N/A',
					undefined,
					vscode.TreeItemCollapsibleState.None,
				),
			);
			attackItem.children.push(
				new MetaDataInfoItem(
					'Body Location:',
					this.web.attack.evidence?.bodyLocation?.startLine || 'N/A',
					undefined,
					vscode.TreeItemCollapsibleState.None,
				),
			);
