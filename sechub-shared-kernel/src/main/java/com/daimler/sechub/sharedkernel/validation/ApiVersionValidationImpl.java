...
public class ApiVersionValidationImpl extends AbstractSimpleStringValidation implements ApiVersionValidation {
	...
	void validateSupportedVersion(ValidationContext<String> context) {
		...
		if (string == null) {
			context.addError("There is no API version defined (null) so not valid, supported is:"+createSupportedApis());
			return;
		}
		...
		context.addError("Given API "+string+" is not supported. Supported versions are:"+createSupportedApis());
	}
