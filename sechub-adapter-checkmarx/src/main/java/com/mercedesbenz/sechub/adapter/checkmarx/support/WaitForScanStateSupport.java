...
class WaitForScanStateSupport extends WaitForStateSupport<CheckmarxContext, CheckmarxAdapterConfig> {
	...
	private void fetchScanDetails(CheckmarxContext context) throws AdapterException {
		...
		try {
			...
			ResponseEntity<String> queueData = restTemplate.getForEntity(context.getAPIURL("sast/scans/" + scanId),
					String.class);
			...
			if (HttpStatus.NOT_FOUND.equals(e.getStatusCode())) {
				...
				LOG.info("Unable to find Checkmarx scan Id: {}. Possible reasons: no longer in queue or never existed.",
						scanId);
