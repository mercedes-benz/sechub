...
function usage(){
    log "usage: integrationtest-pds <cmd=start|stop|waitForStop|waitForAlive|status|checkAlive> {pdsVersion>}|{<pdsPort>}{sharedTempSharedVolumeFolder}" 
    log "       (PDS version is only necessary for start command" 
...
function checkAlive(){
    ...
    if [ $SECHUB_ITS_ALIVE_HTTP_STATUS -eq 200 ]; then
        log "Integration test PDS is alive"
...
function status(){
    ...
    if [ $SECHUB_ITS_ALIVE_HTTP_STATUS -eq 200 ]; then
        ...
        log "- check alive state=$SECHUB_ITS_ALIVE_HTTP_STATUS"
        log "Integration test PDS is not running"
    fi
}
...
function waitForStop(){
    ...
    until isAlive || [ $loopCount -eq $maxLoop ]; do
        ...
        log "-waited $runningSeconds/$timoutSeconds seconds for other integration test PDS to shutdown"
        loopCount=$((loopCount+1))
    done
    if [ $loopCount -eq $maxLoop ] ; then
        log "wait for other integration PDS failed- time out $timoutSeconds seconds reached. So did not work!"
        log "The other process seems to be in 'zombie state' so we force a stop of other PDS."
        stopServer
        log "-------------------------------------------------"
        log "Concurrency integration test PDS problem happened"
        log "-------------------------------------------------"
...
function stopServer(){
    checkAlive
    if [ $SECHUB_ITS_ALIVE_HTTP_STATUS -eq 200 ]; then
        log "Try to stop Integration test PDS"
    else
        return 0
    fi
...
function startServer(){
    ...
    if [ $SECHUB_ITS_ALIVE_HTTP_STATUS -eq 200 ]; then
        log "A former integration test PDS is still alive."
        stopServer
    fi

    log "starting a sechub-pds $PDS_VERSION in integration test mode"
...
function defineSharedVolumeBasePath(){
    if [ -z "$1" ] ; then
        ...
        log "> no shared PDS volume defined, using fallback:temp"
    else
        SHARED_VOLUME_BASEDIR="$1"
        log "> shared PDS volume defined with: $SHARED_VOLUME_BASEDIR"
    fi
}
...
function handleArguments() {
    ...
    export SERVER_PORT
    log "Using port $SERVER_PORT for integration test PDS"
