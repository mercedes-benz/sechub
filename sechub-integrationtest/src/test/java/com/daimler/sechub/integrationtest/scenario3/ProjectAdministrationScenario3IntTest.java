...
public class ProjectAdministrationScenario3IntTest {
    ...
    public void a_superadmin_can_assign_owner_to_project() {
        assertUser(SUPER_ADMIN).canAssignOwnerToProject(USER_2, PROJECT_1);
    }

    @Test
    public void when_a_superadmin_assigns_an_owner_to_a_project_the_owner_is_assigned() {
        /* check precondition */
        assertUser(USER_2).isNotOwnerOf(PROJECT_1);

        /* execute */
        as(SUPER_ADMIN).assignOwnerToProject(USER_2, PROJECT_1);

        /* test */
        assertUser(USER_2).isOwnerOf(PROJECT_1);
    }
    ...
    public void when_a_superadmin_assigns_an_owner_to_a_project_the_previous_owner_is_still_owner_of_another_project() {
        /* prepare */
        assertUser(USER_1).isOwnerOf(PROJECT_1).isOwnerOf(PROJECT_2).hasOwnerRole();

        /* execute */
        as(SUPER_ADMIN).assignOwnerToProject(USER_2, PROJECT_1);

        /* test */
        assertUser(USER_1).isNotOwnerOf(PROJECT_1).isOwnerOf(PROJECT_2).hasOwnerRole();
    }
    ...
    public void when_a_superadmin_assigns_another_owner_to_all_projects_the_previous_owner_loses_owner_role() {
        ...
        /* execute */
        as(SUPER_ADMIN).assignOwnerToProject(USER_2, PROJECT_1).assignOwnerToProject(USER_2, PROJECT_2);

        /* test */
        assertUser(USER_1).isNotOwnerOf(PROJECT_1).isNotOwnerOf(PROJECT_2).hasNotOwnerRole();
    }

    @Test
    public void when_a_superadmin_assigns_another_owner_to_a_project_all_associated_users_are_mailed() {
        /* prepare */
        assertUser(USER_1).isOwnerOf(PROJECT_1).hasOwnerRole();

        /* execute */
        as(SUPER_ADMIN).assignUserToProject(USER_3, PROJECT_1).assignOwnerToProject(USER_2, PROJECT_1);
