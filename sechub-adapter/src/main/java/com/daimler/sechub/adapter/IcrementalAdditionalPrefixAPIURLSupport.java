 ...
 * automated incrementation. Interesting for wiremock tests to provide always
 * different urls, so this class can be used as a workaround when testing
 * adapters with wiremock (wire mock is not able to handle GET-Requests with
 * request bodies, also the "scenario"-aproach is sometimes a little bit
 * cumbersome when having multiple calls). By using this support you can easily
 * and exactly check the correct ordering and call hierarchy.
 *
 * <pre>
...
public class IcrementalAdditionalPrefixAPIURLSupport extends APIURLSupport {
    ...
    /**
     * Assert current checked position is as expected. This is normally a checked
     * marker in code for easier debugging. So you are sure which index is used for
     * a test / stubbing.
     *
    ...
    public IcrementalAdditionalPrefixAPIURLSupport assertCheck(int expectedCheckIndex) {
        if (expectedCheckIndex != checkingPrefixIndex) {
            throw new IllegalStateException("Expected position was:" + expectedCheckIndex + ", but was:" + checkingPrefixIndex);
        }
        return this;
    }
