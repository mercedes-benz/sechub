...
Doing a manual <<_scan>> the first time using PDS is recommended. However, after some time typing in the commands, which  a scan becomes very tedious.
To improve on the experience one can scan using a script.
...
The cluster is created locally via `docker-compose`. The cluster consists of a PostgreSQL database, a Nginx loadbalancer and one or more PDS server. 
