...
----
SECRET_VALIDATOR_CONNECTIONRETRIES
---- 

[TIP]
====
The property `secret.validator.connection-retries`or the corresponding environment variable `SECRET_VALIDATOR_CONNECTIONRETRIES` can be set to specify the number of retries for each of the request, to be more resistant to network issues.
