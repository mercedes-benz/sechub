...
import com.daimler.sechub.domain.administration.user.InternalInitialDataService;
...
public class InitialAdminInitializer {
	...
	public CommandLineRunner initialIntegrationTestAdmin(InternalInitialDataService internalService) {
		return args -> {
			 ...
			 * Integration tests do not contain any production data and also run only on a non productive system, so in this case
			...
			/* an additional test user, has no rights initial. Only for integration tests, so password here plain and not configurable */
