...
public class UserRepositoryImpl implements UserRepositoryCustom{
	...
	private static final String QUERY_DELETE_PROJECT_TO_USER = "delete from "+User.TABLE_NAME_PROJECT_TO_USER+" p2u where p2u."+User.ASSOCIATE_PROJECT_TO_USER_COLUMN_USER_ID+" = ?1";
	private static final String QUERY_DELETE_USER = "delete from "+User.TABLE_NAME+" u where u."+User.COLUMN_USER_ID+" = ?1";
	private static final String QUERY_COUNT_USER_IS_OWNER_OF_PROJECT = "select count(p."+Project.COLUMN_PROJECT_ID+") from "+Project.TABLE_NAME+" p where p."+Project.COLUMN_PROJECT_OWNER+" = ?1";
	...
	public void deleteUserWithAssociations(String userId) {
		...
		if (number.intValue()>0) {
			throw new NotAcceptableException("User "+userId+" is "+number.intValue()+" times still owner of a project! Move ownership before delete!");
		}
