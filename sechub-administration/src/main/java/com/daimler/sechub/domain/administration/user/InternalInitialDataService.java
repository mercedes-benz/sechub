...
public class InternalInitialDataService {
	 ...
	 * environment this administrator should be deleted when another user has gained administrator rights
	 *
	 * @param userId
	 * @param emailAdress
	 ...
	 * @param userId
	 * @param emailAdress
	 * @param unencryptedAPItoken - use "{nooop}" as prefix to prevent token encryption
	 */
	...
	private void internalCreateInitialUser(String userId, String emailAddress, String unencryptedAPItoken, boolean createAsSuperAdmin) {
		 ...
		 *
		 * Not very smart... but it works and its only done one time at first
		 * start... so acceptable.
		 */
		try {
			Thread.sleep(500); //
	...
	private User createUser(String userId, String emailAddress, String fixApiToken, boolean superAdmin) {
		...
		if (fixApiToken.startsWith("{noop}")) {
			/* special case for noop variants, where no password encrypt is
			 * wanted - e.g. on integration tests bcrypt passwords do slow
	...
	private void logInitialAdminCredentials(String initialAdminUserId, String unencryptedApiToken, String emailAddress) {
		...
		/* TODO de-jcup, 2019-05-29: think about forgetting this log as well - user has an email and can reset his/her account */
	}
