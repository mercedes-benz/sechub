 ...
 * retrieving an access token response from a configured Identity Provider (IDP)
 ...
 * encoded in Base64 and included in the <code>Authorization</code> header of
 * the request.
 * </p>
 * <p>
 * The response from the IDP is expected to be a
 * {@link LoginOAuth2TokenResponse}, which includes the access token, refresh
 * token, ID token, and the expiration time of the access token.
 * </p>
 *
 * @see OAuth2AccessTokenResponseClient
 * @see OAuth2AuthorizationCodeGrantRequest
 * @see OAuth2AccessTokenResponse
 * @see LoginOAuth2TokenResponse
 *
 * @author hamidonos
 */
class LoginOAuth2AccessTokenClient implements OAuth2AccessTokenResponseClient<OAuth2AuthorizationCodeGrantRequest> {

    private static final Logger LOG = LoggerFactory.getLogger(LoginOAuth2AccessTokenClient.class);
    ...
    LoginOAuth2AccessTokenClient(RestTemplate restTemplate) {
    ...
    public OAuth2AccessTokenResponse getTokenResponse(OAuth2AuthorizationCodeGrantRequest authorizationGrantRequest) {
        ...
        LoginOAuth2TokenResponse loginOAuth2TokenResponse;
        try {
            loginOAuth2TokenResponse = restTemplate.postForObject(tokenUri, entity, LoginOAuth2TokenResponse.class);

            if (loginOAuth2TokenResponse == null) {
                throw new RestClientException("Access token response is null");
            }
        } catch (RestClientException e) {
            String errMsg = "Failed to get access token response";
            LOG.error(errMsg, e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, errMsg, e);
        }

        Map<String, Object> additionalParameters = Map.of(ID_TOKEN, loginOAuth2TokenResponse.getIdToken());

        /* @formatter:off */
        return OAuth2AccessTokenResponse
                .withToken(loginOAuth2TokenResponse.getAccessToken())
                .tokenType(OAuth2AccessToken.TokenType.BEARER)
                .expiresIn(loginOAuth2TokenResponse.getExpiresIn())
                .refreshToken(loginOAuth2TokenResponse.getRefreshToken())
