...
https://github.com/AlDanial/[cloc] is a free and open source application that counts blank lines, comment lines, and physical lines of source code in many programming languages. It is used on the command line. By combining cloc with the SecHub https://mercedes-benz.github.io/sechub/latest/sechub-product-delegation-server.html[Product Delegation Server (PDS)] in a container, cloc can be used with SecHub. Another scenario is to use cloc+PDS standalone.
