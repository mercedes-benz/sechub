...
https://securego.io/[IaC] is a free and open source static application security testing tool. IaC is a command line interface (CLI) tool. By combining IaC with the SecHub https://mercedes-benz.github.io/sechub/latest/sechub-product-delegation-server.html[Product Delegation Server (PDS)] in a container, IaC can be used with SecHub. Another scenario is to use IaC+PDS standalone.

Furthermore, the combination of IaC and PDS makes it possible to run both inside a Kubernetes cluser or virtual machines.
...
export PDS_PRODUCT_IDENTFIER=PDS_IAC
----
+
For example:
+
