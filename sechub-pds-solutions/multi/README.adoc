...
This example contains multiple free and open source static application security testing tool together with the https://daimler.github.io/sechub/latest/sechub-product-delegation-server.html[Product Delegation Server (PDS)]. A list of the included tools can be found below.
...
The combination of GoSec and PDS makes it possible to run both inside a Kubernetes cluser or virtual machines.
...
[NOTE]
We offer Alpine Linux and Ubuntu based images. What you choose is up to you. However, for people with little Linux experience we recommend to use the Ubuntu image. The Alpine Linux image is intended for experienced Linux users.
