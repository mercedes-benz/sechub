...
server () {
    ...
    # As pds user: create a new schema pmd and authorize pds to the schema
    psql --username pds --command="CREATE SCHEMA IF NOT EXISTS pmd AUTHORIZATION pds;" pds

    # Create a new user with a password
    psql --command="CREATE USER $DATABASE_USERNAME WITH PASSWORD '$DATABASE_PASSWORD';"

    # As pds user: give all privileges of the schema pmd to the new user
    psql --username pds --command="GRANT ALL ON SCHEMA pmd TO $DATABASE_USERNAME;"
