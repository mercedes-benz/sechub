...
This folder contains the necessary scripts to run PDS + FindSecurityBugs inside a container locally. Additionally, it contains scripts to build and push the PDS + FindSecurityBugs container to your container registry and a Helm chart to install and run PDS + FindSecurityBugs in a Kubernetes cluster.
...
+
----
docker-compose --file docker-compose_pds_findsecuritybugs.yaml up --build
----
...
----
+
The default values for the FindSecurityBugs + PDS `docker-compose` examples are:
+
----
...
+
----
./pds-api.sh create_job PDS_FINDSECURITYBUGS "288607bf-ac81-4088-842c-005d5702a9e9"
{
...
+
----
./10-create-image.sh my.registry.example.org/sechub/pds_gosec v0.1 "my.registry.example.org/ubuntu:20.04"
----
