...
then
    ...
    echo "No .git folder was found. Perform secret scan on filesystem without history deepscan." | tee "$PDS_JOB_USER_MESSAGES_FOLDER"/nogit.txt
...
then
    gitleaks_options="$gitleaks_options --no-git"
    echo "Multiple .git folders were found. Perform secret scan on filesystem without history deepscan." | tee "$PDS_JOB_USER_MESSAGES_FOLDER"/multiplegit.txt

# If exactly one '.git' directory was found we scan the git history
else
    scan_target_directory="$repository_root_directory"
    echo ".git folder was found. Perform secret scan with history deepscan." | tee "$PDS_JOB_USER_MESSAGES_FOLDER"/historyscan.txt
fi
