...
server () {
    ...
    # As pds user: create a new schema checkmarx and authorize pds to the schema
    psql --username pds --command="CREATE SCHEMA IF NOT EXISTS checkmarx AUTHORIZATION pds;" pds

    # Create a new user with a password
    psql --command="CREATE USER $DATABASE_USERNAME WITH PASSWORD '$DATABASE_PASSWORD';"

    # As pds user: give all privileges of the schema checkmarx to the new user
    psql --username pds --command="GRANT ALL ON SCHEMA checkmarx TO $DATABASE_USERNAME;"
