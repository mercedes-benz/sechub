...
# FIXME de-jcup, 2022-08-09: create_json is still for a web scan. Either we use
#                            here always only a config file or we create
#                            a working code scan configuation here.
...
if [[ ! -z "$source_folder_or_config" ]]
then
    if [[ -f "$source_folder_or_config" ]]
    then
        echo "JSON job configuration provided."
        json_config="$source_folder_or_config"
    else
        echo "Target Url provided."
        target_url="$source_folder_or_config"
    fi
else
    parameter_missing "Parameter Source Folder or JSON config is missing."
fi
