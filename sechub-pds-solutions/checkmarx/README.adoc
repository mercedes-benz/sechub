...
This folder contains the necessary scripts to run the PDS instance inside a container locally. 
Additionally, it contains scripts to build and push the container to your container registry and a 
Helm chart to install and run the PDS in a Kubernetes cluster. + 

https://checkmarx.com/[Checkmarx] is a commercial security tool for SAST. 

By combining Checkmarx with the SecHub https://mercedes-benz.github.io/sechub/latest/sechub-product-delegation-server.html[Product Delegation Server (PDS)] 
and the https://github.com/mercedes-benz/sechub/tree/develop/sechub-wrapper-checkmarx[SecHub Checkmarx Wrapper]  inside in a container, a running Checkmarx environment can be used with SecHub.
...
. Start the Debian container using https://docs.docker.com/compose/[Docker Compose]:
+
----
docker-compose --file docker-compose_pds_checkmarx_debian.yaml up --build
----
...
----
+
The default values for the PDS-Checkmarx `docker-compose` examples are:
+
----
...
+
----
./pds-api.sh create_job PDS_CHECKMARX "288607bf-ac81-4088-842c-005d5702a9e9"
{
...
The cluster uses an object storage to store files. The cluster uses https://github.com/chrislusf/seaweedfs[SeaweedFS] (S3 compatible) to store files. The PDS instance(s) use the object storage to upload files to. Reading, extracting and analysing the files is done in the PDS+Checkmarx container.
...
Starting several PDS + Checkmarx instances
...
----
docker exec -it pds-checkmarx-debian bash
----
...
+
----
./10-create-debian-image.sh my.registry.example.org/sechub/pds_checkmarx v0.1
----

. Using your own base image:
+
----
./10-create-debian-image.sh my.registry.example.org/sechub/pds_checkmarx v0.1 "my.registry.example.org/debian:11-slim"
----
...
image:
   registry: my.registry.org/sechub/pds_checkmarx
   tag: latest
...
pds-checkmarx-759ffc8dfb-8jj8f                    1/1     Running   0          75s
----

. Forward port of one of the pods to own machine
+
----
kubectl port-forward pds-checkmarx-759ffc8dfb-8jj8f 8444:8444
----
...
pds-checkmarx                 	my-namespace	1       	2021-11-05 18:42:23.613991303 +0100 CET 	deployed	pds-checkmarx-0.1.0            	0.24.0 
----
...
+
----
kubectl describe pod pds-checkmarx-759ffc8dfb-8jj8f
…
Events:
  Type    Reason     Age    From               Message
  ----    ------     ----   ----               -------
  Normal  Scheduled  6m52s  default-scheduler  Successfully assigned sechub-zap/pds-checkmarx-759ffc8dfb-8jj8f to c06p043-md-cc8c675cb-bqtpb
  Normal  Pulling    6m52s  kubelet            Pulling image "my.registry.org/sechub/pds_checkmarx:latest"
  Normal  Pulled     6m31s  kubelet            Successfully pulled image "my.registry.org/sechub/pds_checkmarx:latest" in 21.303104727s
  Normal  Created    6m30s  kubelet            Created container pds-checkmarx
  Normal  Started    6m30s  kubelet            Started container pds-checkmarx
----

* Access container logs.
+
----
kubectl logs pds-checkmarx-759ffc8dfb-8jj8f
...
2021-11-05 17:42:47.697  INFO 7 --- [           main] d.s.p.ProductDelegationServerApplication : Starting ProductDelegationServerApplication using Java 11.0.11 on pds-checkmarx-759ffc8dfb-8jj8f with PID 7 (/pds/sechub-pds-0.24.0.jar started by zap in /workspace)
