	...
	public ResilienceProposal consultFor(ResilienceContext context) {
		...
		if (rootCause instanceof HttpClientErrorException) {
			...
			if (statusCode == 400) {
				...
				/* An internal server error happened - lets assume that this is temporary and do a retry*/
				LOG.info("Propose retry for internal server error");
				return new SimpleRetryResilienceProposal("checkmarx internal server error handling", internalServerErrortMaxRetries, internalServerErrorTimeToWaitInMilliseconds);
