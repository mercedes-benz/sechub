 ...
 * 
 * Because AE algorithms do both, encrypt and hash at the same time they are difficult to misuse.
 *  
 * Furthermore, it is impossible to ensure a nonce (initialization vector) is not used more than once, 
 * therefore a nonce misuse-resistant algorithm is required.
 *  
 * In theory one could make sure, that there is a single data entry point which keeps track of the used nonces.
 * Before writing the data to the data base, the single data entry point generates a nonce and looks up into the 
 ...
 * distributed application impossible.
 * 
 * To avoid those problems, a nonce misuse-resistant algorithm is required. A nonce misuse-algorithm provides security 
 * even if the same combination of secret key, nonce and plain text is encrypted and stored more than once.
 * 
 ...
 * - COLM
 * 
 * Symmetric encryption algorithms are not in danger to be completely broken by quantum computers (see: https://security.stackexchange.com/a/103560).
 * However, it might be necessary to use different cryptographic algorithms in the future as attacks on the existing algorithms become possible.
 * 
 * Bruce Schneier put it this way:
 * 
 ...
 * Key rotation is used to change the secret key. Algorithm rotation can be used if the algorithms need to be exchanged.
 */
