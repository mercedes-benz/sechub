...
- switch `test-with-integrationtests` to `test` and rename former `test` script entry to something else - e.g. `test-without-integrationtests`
...
- switch `test` script entry back to `test-with-integrationtests` and `test-without-integrationtests` to `test`.
