...
- uses: mercedes-benz/sechub/github-actions/scan@master
  with:
    ...
    # OPTIONAL: File that defines false positives. This step is executed before the scan. The SecHub cli defineFalsePositives overwrites the false positives on SecHub side with the ones defined in this file. Use the SecHub cli getFalsePositives to get the current false positives and extend the returned false positive list.
