...
class AwsS3JobStorageTest {
	...
	void store_name_null() {
		...
		/* execute + test */
        assertThrowsExceptionContainingMessage(NullPointerException.class, "name may not be null!",
	...
	void store_input_stream_null() {
		...
		/* execute + test */
        assertThrowsExceptionContainingMessage(NullPointerException.class, "inputStream may not be null!",
                () -> jobStorage.store(fileName, input, 0));
	}
