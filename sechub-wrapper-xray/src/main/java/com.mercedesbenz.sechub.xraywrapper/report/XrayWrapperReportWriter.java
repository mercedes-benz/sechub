...
public class XrayWrapperReportWriter {
    ...
    /*
     * public static void writeReport(ObjectNode rootObject, File report) throws
     * XrayWrapperReportException { ObjectMapper mapper = new ObjectMapper();
     * ObjectWriter writer = mapper.writer(new DefaultPrettyPrinter()); try {
     * writer.writeValue(new File(report.toURI()), rootObject); } catch (IOException
     * e) { throw new
     * XrayWrapperReportException("Could not write final xray report to file", e,
     * XrayWrapperExitCode.IO_ERROR); } }
     *
     */
